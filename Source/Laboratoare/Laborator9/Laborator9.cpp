...
void Laborator9::Init()
{
	...
	{
		Texture2D* texture = new Texture2D();
		texture->Load2D((textureLoc + "Side1.png").c_str(), GL_REPEAT);
		mapTextures["side1"] = texture;
	}
	...
	{
		Mesh* mesh = new Mesh("obamaprism");
		mesh->LoadMesh(textureLoc, "Obamium.obj");
		meshes[mesh->GetMeshID()] = mesh;
	}
...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		modelMatrix = glm::translate(modelMatrix, glm::vec3(2, 0.5f, 3));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
		RenderSimpleMesh(meshes["obamaprism"], shaders["ShaderLab9"], modelMatrix, mapTextures["earth"]);
	}
}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	// update the elapsed time used by the sphere; other meshes don't use it
	glUniform1f(glGetUniformLocation(shader->program, "elapsed_time"), mesh == meshes["sphere"] ? Engine::GetElapsedTime() : 0);

	// instruct the shader to mix the textures
	glUniform1i(glGetUniformLocation(shader->program, "use_2_textures"), texture1 && texture2);
